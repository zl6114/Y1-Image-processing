    ...
    ac_int<PIXEL_WL, false> * video_in, ac_int<PIXEL_WL, false> * video_out)
{
	...
	//ac_int<10, false> histogramGreen;
	//ac_int<10, false> histogramRed;
	//ac_int<10, false> histogramBlue;
	//ac_int<10, false> i;
...
*/    
    ...
    /// something here...
    /*if ((vga_x == mouse_x + cursor_size) && (vga_y == mouse_y + cursor_size)){
		for(i = vga_x; i <= (vga_x + cursor_size); i++){
			histogramGreen = (histogramGreen + i_green)/2;
			histogramRed = (histogramRed + i_red)/2;
			histogramBlue = (histogramBlue + i_blue)/2;
		}
    }*/
    ...
    if ((vga_x >= mouse_x - cursor_size) && (vga_x <= mouse_x + cursor_size) && (vga_y >= mouse_y - cursor_size) && (vga_y <= mouse_y + cursor_size)){
		...
		ac_int<10,false> intensity=(i_red+i_green+i_blue)/3;
